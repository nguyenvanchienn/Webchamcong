...
const Schedules: React.FC<{ mode?: 'REGISTER' | 'CREATE' }> = ({ mode = 'REGISTER' }) => {
  ...
  const fetchData = async (silent = false) => {
    ...
    try {
      ...
      if (settingsDoc.exists()) {
        ...
        if (data.openTimes) setOpenTimes(data.openTimes);
        if (data.previewTimes) setPreviewTimes(data.previewTimes);
      }
    } catch (error) {
  ...
  const handleSaveDeadline = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      newDeadlines[monday] = localDeadline;
      if (localOpenTime) newOpenTimes[monday] = localOpenTime;
      else delete newOpenTimes[monday];
