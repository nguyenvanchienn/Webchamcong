...
const applyStylesToSheet = (ws: XLSX.WorkSheet, boldKeywords: string[]) => {
  ...
  for (let C = range.s.c; C <= range.e.c; ++C) {
    ...
    ws[cellRef].s = { font: { bold: true }, fill: { fgColor: { rgb: "EFEFEF" } } };
  }
  ...
  for (let R = 1; R <= range.e.r; ++R) {
    ...
    for (let C = range.s.c; C <= range.e.c; ++C) {
      ...
      if (cell && typeof cell.v === 'string' && boldKeywords.some(kw => cell.v.includes(kw))) {
        isBoldRow = true;
        break;
      }
    }
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      sortedEmployees.forEach((emp) => {
        ...
        // Group by Date
        const datesSet = new Set([...empAtts.map(a => a.date), ...empSchs.map(s => s.date)]);
        ...
        sortedDates.forEach(date => {
           ...
           if (attsOnDate.length > 0) {
             attsOnDate.forEach(data => {
               ...
               if (data.logs && data.logs.length > 3) status += ' (Có ngắt quãng)';
               ...
               excelData.push({
                 ...
                 'Mức lương (VNĐ/h)': '',
                 'Thành tiền (VNĐ)': earned > 0 ? new Intl.NumberFormat('vi-VN').format(earned) : '',
        ...
        empBonuses.forEach(b => {
           ...
           excelData.push({
             ...
             'Ngày': b.createdAt?.toDate ? b.createdAt.toDate().toLocaleDateString('vi-VN') : '',
             'Check In': '',
             'Check Out': '',
             'Số giờ': '',
             'Mức lương (VNĐ/h)': '',
             'Thành tiền (VNĐ)': (isPenalty ? '-' : '+') + new Intl.NumberFormat('vi-VN').format(amt),
        ...
        excelData.push({
          ...
          'Mức lương (VNĐ/h)': new Intl.NumberFormat('vi-VN').format(emp.salaryPerHour),
          'Thành tiền (VNĐ)': new Intl.NumberFormat('vi-VN').format(Math.round(employeeTotalEarned + employeeTotalBonus - employeeTotalPenalty)),
          'Trạng thái / Lý do': `Lương: ${new Intl.NumberFormat('vi-VN').format(Math.round(employeeTotalEarned))}, Thưởng: ${new Intl.NumberFormat('vi-VN').format(employeeTotalBonus)}, Phạt: ${new Intl.NumberFormat('vi-VN').format(employeeTotalPenalty)}`
        });
        ...
        excelData.push({
          'STT': '', 'Mã NV': '', 'Họ và Tên': '', 'Chức vụ': '', 'Cơ sở': '', 'Phân loại': '', 
          'Ngày': '', 'Check In': '', 'Check Out': '', 'Số giờ': '', 'Mức lương (VNĐ/h)': '', 'Thành tiền (VNĐ)': '', 'Trạng thái / Lý do': ''
        });
      });
      ...
      applyStylesToSheet(worksheet, ['TỔNG KẾT THÁNG']);
    ...
    try {
      ...
      ordersSnap.forEach(doc => {
        ...
        if (orderDate >= startOfMonth && orderDate < endOfMonth) {
           if (filterBranchId !== 'ALL' && data.branchId !== filterBranchId) return;

           const isExpense = data.type === 'EXPENSE';
           const itemsStr = data.items ? data.items.map((i: any) => `${i.name} (x${i.quantity})`).join(', ') : '';
           const content = isExpense ? (data.note || itemsStr) : itemsStr;
           const amount = data.totalAmount || 0;
           const branchName = data.branchId ? branchMap[data.branchId] || 'Chưa rõ' : 'Chưa rõ';
           ...
           rawData.push({
             ...
             paymentMethod: data.paymentMethod === 'TRANSFER' ? 'Chuyển khoản' : 'Tiền mặt',
             content,
             amount
           });
        }
      ...
      const empSnap = await getDocs(collection(db, 'employees'));
      const employees: Record<string, any> = {};
      empSnap.forEach(d => { employees[d.id] = { id: d.id, ...d.data() }; });
      
      const startDateStr = `${year}-${m}-01`;
      const endDateStr = new Date(parseInt(year), parseInt(m), 0).toLocaleDateString('en-CA');
      
      const attSnap = await getDocs(query(collection(db, 'attendance'), where('date', '>=', startDateStr), where('date', '<=', endDateStr)));
      const bonusSnap = await getDocs(query(collection(db, 'bonuses'), where('month', '==', month)));
      ...
      Object.values(employees).forEach(emp => {
         ...
         const empAtts = attSnap.docs.map(d=>d.data()).filter(d=>d.employeeId === emp.id);
         ...
         const empBonuses = bonusSnap.docs.map(d=>d.data()).filter(b=>b.employeeId === emp.id);
         empBonuses.forEach(b => {
         ...
         if (netSalary > 0) {
            const bName = emp.branchName || (emp.branchId ? branchMap[emp.branchId] : null) || 'Chưa rõ';
            const isMatch = filterBranchId === 'ALL' || emp.branchId === filterBranchId || bName === branchMap[filterBranchId];
            if (isMatch) {
               rawData.push({
                 id: `SALARY-${emp.id}`,
      ...
      Object.keys(branchGroups).sort().forEach(bName => {
         let branchIncome = 0;
         let branchExpense = 0;
         
         const sortedBranchRows = branchGroups[bName].sort((a, b) => a.orderDate.getTime() - b.orderDate.getTime());
         ...
         sortedBranchRows.forEach(row => {
            ...
            excelData.push({
               ...
               'Số tiền (VNĐ)': (row.isExpense ? '-' : '+') + new Intl.NumberFormat('vi-VN').format(row.amount)
            });
         });
         ...
         excelData.push({
            'STT': '', 'Mã GD': '', 'Cơ sở': bName, 'Loại': 'TỔNG CƠ SỞ', 'Ngày giờ': '', 'Thu ngân': '', 'Hình thức': '',
            ...
            'Số tiền (VNĐ)': new Intl.NumberFormat('vi-VN').format(branchIncome - branchExpense)
         });
         
         excelData.push({ 'STT': '', 'Mã GD': '', 'Cơ sở': '', 'Loại': '', 'Ngày giờ': '', 'Thu ngân': '', 'Hình thức': '', 'Nội dung': '', 'Số tiền (VNĐ)': '' });
      });

      if (filterBranchId === 'ALL') {
         excelData.push({
            'STT': '', 'Mã GD': '', 'Cơ sở': 'TẤT CẢ CƠ SỞ', 'Loại': 'TỔNG CỘNG', 'Ngày giờ': '', 'Thu ngân': '', 'Hình thức': '',
            ...
            'Số tiền (VNĐ)': new Intl.NumberFormat('vi-VN').format(totalAllIncome - totalAllExpense)
         });
      }
