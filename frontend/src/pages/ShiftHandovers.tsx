...
const ShiftHandovers = () => {
  ...
  const [filterBranchId, setFilterBranchId] = useState(
    userRole === "BRANCH_ADMIN" ? currentBranchId : "all",
  );
  ...
  const [posActiveEmployees, setPosActiveEmployees] = useState<
    { id: string; name: string; email: string; isCashier: boolean }[]
  >([]);
  const [selectedPosEmployeeId, setSelectedPosEmployeeId] =
    useState<string>("");
  ...
  const fetchData = useCallback(
      ...
      try {
        ...
        snap.forEach((doc) => {
          ...
          if (
            ...
            return;
          if (userRole === "BRANCH_ADMIN" && data.branchId !== currentBranchId)
            return;
          ...
          if (report.status === "OPEN") {
            ...
            openShiftPromises.push(
              (async () => {
                const rev = await calculateRevenue(
                  report.startTime,
                  new Date(),
                  report.branchId,
                );
    ...
    [userRole, currentUserEmail, currentBranchId, filterBranchId, filterDate],
  );
  ...
  const calculateRevenue = async (start: Date, end: Date, branchId: string) => {
    try {
      ...
      ordersSnap.forEach((doc) => {
        const data = doc.data();
        if (data.branchId === branchId && data.status === "COMPLETED") {
          // Lọc theo branchId
  ...
  const handleOpenShift = async () => {
    ...
    try {
      if (
        (userRole === "SUPER_ADMIN" || userRole === "BRANCH_ADMIN") &&
        (!branchIdToUse || branchIdToUse === "all")
      ) {
      ...
      if (branchIdToUse) {
        ...
        if (!checkSnap.empty) {
          toast.error(
            "Đang có một ca chưa được chốt! Vui lòng chốt ca hiện tại trước khi mở ca mới.",
          );
          return;
        }
      }
    ...
    if (branchIdToUse) {
      try {
        ...
        if (!hasPos) {
          toast.error(
            "Cơ sở này chưa có Máy Order. Vui lòng vào Quản lý tài khoản để tạo tài khoản Máy Order cho cơ sở này trước!",
          );
          return;
        }
      } catch (err) {
    ...
    if (userRole === "CASHIER" || userRole === "EMPLOYEE") {
      ...
      if (!empId) {
        toast.error(
          "Tài khoản chưa liên kết với nhân viên. Vui lòng liên hệ Quản lý.",
        );
        return;
      }
      try {
        ...
        if (!hasActiveCheckIn) {
          toast.error(
            "Bạn phải Chấm công (Check-in) trước khi được phép Mở ca!",
          );
          return;
        }
      } catch (err) {
      ...
      try {
        ...
        const checkedInIds = snap.docs
          .map((d) => {
            ...
            return data.checkIn &&
              !data.checkOut &&
              (!branchIdToUse || data.branchId === branchIdToUse)
              ? data.employeeId
              : null;
          })
          .filter(Boolean);
        ...
        for (const id of checkedInIds) {
          ...
          if (empDoc.exists()) {
            ...
            const isCashier =
              data.position && data.position.toLowerCase().includes("thu ngân");
            activeEmps.push({
              id: id as string,
        ...
        if (activeEmps.length === 0) {
          toast.error(
            "Không có nhân viên nào đang check-in. Vui lòng chấm công trước khi mở ca!",
          );
          return;
        }
    ...
    try {
      if (modalMode === "OPEN") {
        ...
        if (
          ...
          const selectedEmp = posActiveEmployees.find(
            (e) => e.id === selectedPosEmployeeId,
          );
          if (!selectedEmp) {
          ...
          if (currentEmployeeId) {
            const empDoc = await getDoc(
              doc(db, "employees", currentEmployeeId),
            );
            if (empDoc.exists()) {
            ...
            cashierDisplayName = userSnap.empty
              ? currentUserEmail
              : userSnap.docs[0].data().fullName || currentUserEmail;
          }
        }
        ...
        if (activeShift.startCash !== numStartCash) {
          changes.push(
            `Tiền mặt đầu ca: ${activeShift.startCash} -> ${numStartCash}`,
          );
        }
        if (activeShift.startTransfer !== numStartTransfer) {
          changes.push(
            `Tiền CK đầu ca: ${activeShift.startTransfer} -> ${numStartTransfer}`,
          );
        }
        if (activeShift.endCash !== numEndCash) {
          changes.push(
            `Tiền mặt cuối ca: ${activeShift.endCash} -> ${numEndCash}`,
          );
        }
        if (activeShift.endTransfer !== numEndTransfer) {
          changes.push(
            `Tiền CK cuối ca: ${activeShift.endTransfer} -> ${numEndTransfer}`,
          );
        }
        ...
        if (changes.length > 0) {
          ...
          if (userRole !== "SUPER_ADMIN") {
            if (currentEmployeeId) {
              ...
              editorDisplayName = userSnap.empty
                ? currentUserEmail
                : userSnap.docs[0].data().fullName || currentUserEmail;
            }
          }
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white p-4 rounded-xl shadow-sm border border-gray-100 gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">
            Quản lý đóng/mở ca và đối soát tiền mặt thu ngân
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-1 border border-gray-300 rounded-lg px-1 py-1">
            <button
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div>
                ...
                <p className="text-blue-100 text-sm">
                  Bắt đầu lúc:{" "}
                  {activeShift.startTime.toLocaleTimeString("vi-VN")} (
                  {activeShift.startTime.toLocaleDateString("vi-VN")})
                </p>
              </div>
              <div className="w-full md:w-auto md:text-right">
                <p className="text-blue-100 text-sm mb-2 md:mb-1">
                  Tiền nhận đầu ca
                </p>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                <th className="p-3 font-semibold text-gray-600 text-sm text-center w-10">
                  STT
                </th>
                ...
                {(userRole === "SUPER_ADMIN" ||
                  userRole === "BRANCH_ADMIN") && <th className="p-3"></th>}
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                reports.map((r, index) => {
                  const varianceCash =
                    (r.endCash ?? 0) - (r.startCash + (r.revenueCash ?? 0));
                  const varianceTransfer =
                    (r.endTransfer ?? 0) -
                    (r.startTransfer + (r.revenueTransfer ?? 0));
                  return (
                    <tr
                      key={r.id}
                    ...
                    >
                      ...
                      <td className="p-4">
                        <div className="font-medium text-gray-800">
                          {r.startTime.toLocaleTimeString("vi-VN")}
                        </div>
                        <div className="text-xs text-gray-500">
                          {r.startTime.toLocaleDateString("vi-VN")}
                        </div>
                        {r.endTime && (
                          <div className="text-xs text-gray-400 mt-1">
                            Đến: {r.endTime.toLocaleTimeString("vi-VN")}
                          </div>
                        )}
                      </td>
                      <td className="p-4">
                      ...
                      <td className="p-4 text-sm">
                        <div className="text-gray-800">
                          {formatMoney(r.startCash)}
                        </div>
                        <div className="text-gray-500 text-xs">
                          {formatMoney(r.startTransfer)}
                        </div>
                      </td>
                      <td className="p-4 text-sm">
                        <div className="text-green-600">
                          {formatMoney(r.revenueCash || 0)}
                        </div>
                        <div className="text-gray-500 text-xs">
                          {formatMoney(r.revenueTransfer || 0)}
                        </div>
                      </td>
                      <td className="p-4 text-sm font-medium">
                        <div className="text-gray-800">
                          {r.status === "CLOSED"
                            ? formatMoney(r.endCash || 0)
                            : "---"}
                        </div>
                        <div className="text-gray-500 text-xs">
                          {r.status === "CLOSED"
                            ? formatMoney(r.endTransfer || 0)
                            : "---"}
                        </div>
                      </td>
                      <td className="p-4">
                        {r.status === "CLOSED" ? (
                          <div className="flex flex-col gap-1">
                            <span
                              className={`px-2 py-1 inline-block text-center rounded text-[11px] font-bold ${varianceCash > 0 ? "bg-blue-100 text-blue-700" : varianceCash < 0 ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
                            >
                              TM: {varianceCash > 0 ? "+" : ""}
                              {formatMoney(varianceCash)}
                            </span>
                            <span
                              className={`px-2 py-1 inline-block text-center rounded text-[11px] font-bold ${varianceTransfer > 0 ? "bg-blue-100 text-blue-700" : varianceTransfer < 0 ? "bg-red-100 text-red-700" : "bg-green-100 text-green-700"}`}
                            >
                              CK: {varianceTransfer > 0 ? "+" : ""}
                              {formatMoney(varianceTransfer)}
                            </span>
                          </div>
                        ) : (
                        ...
                        {r.notes && (
                          <div
                            className="text-[10px] text-gray-500 mt-1 max-w-[150px] truncate"
                            title={r.notes}
                          >
                            {r.notes}
                          </div>
                      ...
                      <td className="p-4 text-center">
                        <span
                          className={`px-2 py-1 rounded-full text-[10px] font-bold ${r.status === "OPEN" ? "bg-orange-100 text-orange-700 border border-orange-200" : "bg-gray-100 text-gray-600 border border-gray-200"}`}
                        >
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            <div
              className={`p-4 border-b flex justify-between items-center text-white ${modalMode === "OPEN" ? "bg-blue-600" : modalMode === "EDIT" ? "bg-orange-500" : "bg-green-600"}`}
            >
            ...
            >
                ...
                posActiveEmployees.length > 0 && (
                  <div className="mb-4 bg-blue-50 p-4 rounded-xl border border-blue-100">
                    ...
                    <p className="text-xs text-blue-700 mt-2 italic">
                      {userRole === "POS"
                        ? "Chỉ hiển thị các nhân viên đang check-in trong ca (Ưu tiên thu ngân)."
                        : "Nếu không có ai check-in, ca sẽ mặc định gắn cho bạn."}
                    </p>
                  </div>
                )}
                ...
                <>
                  <div className="bg-blue-50 p-3 rounded-lg border border-blue-100 mb-4">
                    <p className="text-sm text-blue-800">
                      Vui lòng kiểm tra và nhập chính xác số tiền quỹ đầu ca.
                    </p>
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      required
                      value={
                        typeof startCash === "number"
                          ? startCash.toLocaleString("vi-VN")
                          : startCash
                      }
                      onChange={(e) =>
                        handleMoneyInput(e.target.value, setStartCash)
                      }
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      Số dư Tài khoản nhận (nếu có theo dõi)
                    </label>
                    <input
                      type="text"
                      value={
                        typeof startTransfer === "number"
                          ? startTransfer.toLocaleString("vi-VN")
                          : startTransfer
                      }
                      onChange={(e) =>
                        handleMoneyInput(e.target.value, setStartTransfer)
                      }
                      className="w-full px-4 py-3 border border-gray-300 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-lg font-bold text-gray-800"
                    />
                  </div>
                    ...
                    <>
                      <div>
                        ...
                        <input
                          type="text"
                          value={
                            typeof endCash === "number"
                              ? endCash.toLocaleString("vi-VN")
                              : endCash
                          }
                          onChange={(e) =>
                            handleMoneyInput(e.target.value, setEndCash)
                          }
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                          Tiền tài khoản cuối ca
                        </label>
                        <input
                          type="text"
                          value={
                            typeof endTransfer === "number"
                              ? endTransfer.toLocaleString("vi-VN")
                              : endTransfer
                          }
                          onChange={(e) =>
                            handleMoneyInput(e.target.value, setEndTransfer)
                          }
                          className="w-full px-4 py-3 border border-gray-300 rounded-xl outline-none focus:ring-2 focus:ring-orange-500 focus:border-orange-500 text-lg font-bold text-gray-800"
                        />
                      </div>
                ...
                <>
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div className="bg-gray-50 p-3 rounded-xl border border-gray-200">
                      ...
                      <p className="font-bold text-gray-800">
                        {formatMoney(activeShift?.startCash || 0)}
                      </p>
                    </div>
                    <div
                      ...
                      onClick={() => {
                        if (
                          ...
                          navigate("/dashboard/revenue?paymentMethod=CASH");
                        else if (userRole === "POS")
                          navigate("/dashboard/orders?paymentMethod=CASH");
                      }}
                    >
                      <p className="text-xs text-gray-500 mb-1">
                        Doanh thu tiền mặt (tạm tính)
                      </p>
                      <p className="font-bold text-green-600">
                        +{formatMoney(tempRevenueCash)}
                      </p>
                    </div>
                    <div
                      className={`bg-gray-50 p-3 rounded-xl border border-gray-200 ${userRole === "SUPER_ADMIN" || userRole === "BRANCH_ADMIN" || userRole === "POS" ? "cursor-pointer hover:bg-gray-100" : ""}`}
                      onClick={() => {
                        if (
                          ...
                          navigate("/dashboard/revenue?paymentMethod=TRANSFER");
                        else if (userRole === "POS")
                          navigate("/dashboard/orders?paymentMethod=TRANSFER");
                      }}
                    >
                      <p className="text-xs text-gray-500 mb-1">
                        Doanh thu chuyển khoản (tạm tính)
                      </p>
                      <p className="font-bold text-green-600">
                        +{formatMoney(tempRevenueTransfer)}
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    {(() => {
                      ...
                      return (
                        <p
                          className={`text-xs mt-1 font-bold text-center ${variance > 0 ? "text-blue-600" : "text-red-600"}`}
                        >
                          {variance > 0 ? "THỪA" : "THIẾU"}{" "}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">
                      Tiền tài khoản THỰC TẾ cuối ca
                    </label>
                    <input
                      type="text"
                      value={
                    ...
                    {(() => {
                      const theoretical =
                        (activeShift?.startTransfer || 0) + tempRevenueTransfer;
                  ...
                  <div className="bg-blue-50 p-4 rounded-xl border border-blue-200 mt-4 text-center shadow-inner">
                    <p className="text-sm font-medium text-blue-800 mb-1">
                      TỔNG TIỀN THỰC TẾ (MẶT + CK)
                    </p>
                    <p className="text-2xl font-black text-blue-700">
                      {formatMoney(
                        (Number(endCash) || 0) + (Number(endTransfer) || 0),
                      )}
                    </p>
                    {(() => {
                      const theoCash =
                        ...
                        <div className="mt-3 pt-3 border-t border-blue-200/50 text-xs font-medium flex justify-between px-2">
                          <p className="text-blue-700/70">
                            ...
                            <span className="text-sm">
                              {formatMoney(totalTheo)}
                            </span>
                          </p>
                          <div className="text-right">
                            <p className="text-blue-700/70 mb-0.5">
                              Tổng chênh lệch:
                            </p>
                            {totalVariance === 0 ? (
                              ...
                              <p
                                className={`font-bold text-sm ${totalVariance > 0 ? "text-blue-600" : "text-red-600"}`}
                              >
                                {totalVariance > 0 ? "THỪA" : "THIẾU"}{" "}
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Ghi chú{" "}
                  {(modalMode === "CLOSE" || modalMode === "EDIT") &&
                    "(Lý do chênh lệch/chỉnh sửa)"}
                </label>
                <textarea
                  value={notes}
                  ...
                  placeholder={
                    modalMode === "OPEN"
                      ? "Ghi chú thêm..."
                      : "Giải trình lý do chênh lệch (nếu có)..."
                  }
                ></textarea>
              </div>
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-[110] p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden flex flex-col">
            <div className="p-4 border-b bg-blue-600 text-white flex justify-between items-center">
              ...
              <button
                onClick={() => setShowPosPasswordDialog(false)}
            ...
            <form onSubmit={handleVerifyPasswordAndSubmit} className="p-6">
              ...
              <div className="relative mb-6">
                ...
                <button
                  type="button"
                  onClick={() =>
                    setShowPosPasswordVisible(!showPosPasswordVisible)
                  }
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 focus:outline-none"
                >
                  {showPosPasswordVisible ? (
                    <EyeOff size={20} />
                  ) : (
                    <Eye size={20} />
                  )}
        ...
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto bg-gray-50 flex-1">
              {historyShift.editHistory &&
              historyShift.editHistory.length > 0 ? (
                <div className="space-y-6">
