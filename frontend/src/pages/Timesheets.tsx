  ...
  const fetchTimesheets = async () => {
    ...
    try {
      ...
      for (const data of recordsMap.values()) {
        const shiftStr =
          schedulesMap[`${data.employeeId}_${data.date}`] ||
          (data.assignedRole
            ? `Ca được giao\n(bởi ${data.assignedRole})`
            : "Ca được giao");

        let latePenaltyAmount = 0;
        const branchIdForRecord =
          data.branchId ||
          allEmps[data.employeeId]?.branchId ||
          currentUserBranchId;
        ...
        if (
          data.checkIn &&
          shiftStr !== "Ca được giao" &&
          !shiftStr.includes("Ca được giao")
        ) {
          const inTime = data.checkIn.toDate
            ? data.checkIn.toDate()
            : new Date(data.checkIn);
      ...
      records.forEach((r) => {
        ...
        r.salaryPerHour = Number(
          r.salaryPerHour || allEmps[r.employeeId]?.salaryPerHour || 0,
        );
      ...
      <div className="bg-white p-4 md:p-6 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Quản lý và theo dõi giờ làm việc của nhân viên theo tháng.
          </p>
        </div>
                            ...
                            <td colSpan={5} className="p-0">
                              <div className="p-6 border-b border-gray-100">
                                <h4 className="text-sm font-bold text-gray-700 mb-3 flex items-center">
                                  ...
                                  />{" "}
                                  Chi tiết chấm công tháng {month.split("-")[1]}
                                </h4>
                                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                                  {summary.records.map((r) => {
                                    const inTime = r.checkIn
                                      ? r.checkIn
                                          .toDate()
                                    ...
                                    return (
                                      <div
                                        key={r.id}
                                        className="bg-white p-3 rounded border border-gray-200 shadow-sm flex flex-col"
                                      >
                                        <div className="flex justify-between items-center mb-2">
                                          <div className="flex flex-col">
                                            ...
                                            <span className="text-[10px] text-gray-500 whitespace-pre-line leading-tight">
                                              {r.branchName} • {r.shiftStr}
                                            </span>
                                          </div>
                                          <span
                                            className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${
                                              r.status.includes("Đi muộn")
                                          ...
                                          <div className="flex flex-col">
                                            ...
                                            {r.salaryPerHour !== undefined && (
                                              ...
                                              >
                                                ...
                                                {r.latePenaltyAmount ? (
                                                  <span className="text-red-500 text-[9px] mt-0.5">
                                                    (-
                                                    {r.latePenaltyAmount.toLocaleString(
                                                      "vi-VN",
                                                    )}
                                                    đ phạt đi muộn)
                                                  </span>
