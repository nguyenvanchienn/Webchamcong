...
import { ChefHat, CheckCircle, Clock, X, Maximize, Minimize, MessageSquare, Trash2 } from 'lucide-react';
...
const KitchenDisplay: React.FC = () => {
  ...
  const [actionModal, setActionModal] = useState<{
    type: 'REJECT_REQUEST' | 'REJECT_CANCEL' | 'DELETE_ITEM';
    orderId: string;
    itemId?: string;
    reqId?: string;
    itemName?: string;
  ...
  // Sort logic simplified to avoid any hidden crashes
  const getOrderTimestamp = (order: any) => {
    let ts = Date.now();
    if (order.updatedAt?.toMillis) ts = order.updatedAt.toMillis();
    else if (order.createdAt?.toMillis) ts = order.createdAt.toMillis();
    return ts;
  };
    ...
    <div className="min-h-screen bg-gray-900 text-white font-sans flex flex-col">
      ...
      <header className="bg-gray-800 border-b border-gray-700 p-4 flex justify-between items-center shadow-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-orange-400">MÀN HÌNH BẾP</h1>
            <p className="text-sm text-gray-400 font-medium">{storeName}</p>
          </div>
        </div>
        ...
        {allOrders.length === 0 ? (
          ...
          <div className="flex flex-col gap-6 pb-4">
            {allOrders.map((order: any, orderIndex: number) => (
              <div 
                key={order.id} 
                className="bg-gray-800 rounded-2xl border border-gray-700 shadow-xl flex flex-col overflow-hidden"
              >
                {/* Card Header */}
                <div className={`p-4 border-b ${order.type === 'TABLE' ? 'border-blue-900 bg-blue-900/30' : 'border-purple-900 bg-purple-900/30'}`}>
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-4">
                      ...
                      <h3 className={`font-black text-3xl ${order.type === 'TABLE' ? 'text-blue-400' : 'text-purple-400'}`}>
                        {order.type === 'TABLE' ? order.tableName : `Mang đi #${order.orderCode?.slice(-4) || ''}`}
                      </h3>
                    </div>
                ...
                <div className="p-4">
                  ...
                  {order.customerRequests?.filter((r:any) => !r.isCompleted).length > 0 && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3 mb-4">
                      {order.customerRequests.filter((r:any) => !r.isCompleted).map((req: any, idx: number) => (
                        <div key={req.id || idx} className="bg-orange-500/20 hover:bg-orange-500/30 border border-orange-500/30 rounded-xl p-3 transition-all flex flex-col gap-2">
                      <div className="flex justify-between items-start gap-2">
                        <p className="font-bold text-orange-300 leading-tight">
                          {req.message}
                        </p>
                  ...
                  {order.items.map((item: any) => {
                    ...
                    return (
                      <div 
                        key={itemId}
                        className={`border rounded-xl p-3 transition-all group flex flex-col gap-2 ${
                        ...
                        <div className="flex justify-between items-center">
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <span className={`font-black text-xl text-white w-8 h-8 flex items-center justify-center rounded-md border ${
                                item.cancelRequested ? 'bg-red-900/50 border-red-500/50' : item.isServed ? 'bg-gray-800 border-gray-700 text-gray-400' : 'bg-gray-900 border-gray-600'
                              }`}>
                                {item.quantity}
                              </span>
                              <div>
                              ...
                              <button 
                                onClick={(e) => {
                                  ...
                                  setActionModal({
                                    type: 'DELETE_ITEM',
                                    orderId: order.id,
                                    itemId: isTakeaway ? item.menuItemId : item.cartItemId,
                                    itemName: item.name,
                                    reqMessage: item.selectedSize || null // Use reqMessage to pass selectedSize for Takeaway
                                  });
                                }}
                ...
                <div className="p-3 border-t border-gray-700 bg-gray-800 flex justify-end">
                  <button
                    onClick={() => {
                      if (order.type === 'TAKEAWAY') markTakeawayOrderDone(order.id);
                      else markTableOrderDone(order.id);
                    }}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-gray-800 rounded-2xl shadow-2xl max-w-sm w-full p-6 border border-gray-700">
            <h3 className="text-xl font-bold text-center text-red-400 mb-2">
              {actionModal.type === 'DELETE_ITEM' ? 'Xác nhận huỷ món' : 'Xác nhận từ chối'}
            </h3>
